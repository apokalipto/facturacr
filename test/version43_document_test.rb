...
class Version43DocumentTest < Minitest::Test
  ...
  def test_invoice_xml_validation
    ...
    summary = FE::Document::Summary.new currency: "USD", exchange_rate: 575, services_exent_total: 100 , services_exonerate_total: 100, goods_taxable_total: 100, goods_exonerate_total: 1000, exent_total: 100, taxable_total: 100,exonerate_total: 100, subtotal: 200, gross_total: 200, tax_total: 13, net_total: 213

    others = [FE::Document::OtherText.new(xml_attributes: {"code"=>"my123456"}, content: "This is the custom value")]


    invoice = FE::Invoice.new economic_activity: "01",date: Time.now, issuer: issuer, receiver: receiver, number: 1, items: items, condition: "01", summary: summary, other_charges: other_c , security_code: "12345678", document_situation: "1", others: others


  xml=invoice.generate
  ap xml=invoice.generate
      File.open("facturasGeneradas\\file⁩.xml","w"){|f| f.write(xml)}
  end
  ...
  def test_ticket_xml_validation
    ...
    invoice = FE::Ticket.new payment_type: "02", economic_activity: "01", date: Time.now, issuer: issuer, receiver: receiver, number: 1, items: items, condition: "01", summary: summary,other_charges: other_c , security_code: "12345678", document_situation: "1", others: others

    #xml=invoice.generate
#    ap xml=invoice.generate
    #File.open("tiqueteGeneradas\\file⁩.xml","w"){|f| f.write(xml)}
